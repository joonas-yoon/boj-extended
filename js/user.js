function extendUserPage() {
  ...
  function createVsForm(name1, name2) {
    const div = Utils.createElement('div', {class: 'vs', style: 'margin-bottom: 10px'});
    const row = Utils.createElement('div', {class: 'row'});
    const colpad = Utils.createElement('div', {class: 'col col-sr-only col-md-1'});
    ...
    row.appendChild(colpad.cloneNode());
    div.appendChild(row);
    const input1 = Utils.createElement('input', {type: 'text', class: 'form-control', value: name1 || '', placeholder: 'Username'});
    const input2 = Utils.createElement('input', {type: 'text', class: 'form-control', value: name2 || '', placeholder: 'Username'});
    col1.appendChild(input1);
    col2.appendChild(input2);
    const btn = Utils.createElement('button', {type: 'button', class: 'btn btn-primary btn-block'});
    btn.innerText = 'VS';
    btn.addEventListener('click', (evt) => {
      evt.preventDefault();
      const v1 = input1.value; const v2 = input2.value;
      input1.setAttribute('class', 'form-control' + (v1 ? '' : ' text-border-red bg-color-red'));
      input2.setAttribute('class', 'form-control' + (v2 ? '' : ' text-border-red bg-color-red'));
